...
void Require()//进程请求资源
{
	...
	case AVAILABLE_REQUEST:cout << "\n分配失败！申请量超过当前剩余资源数，需等待！！！\n\n"; break;
	case SUCCESS:cout << "\n分配成功！\n"; Display_Security(retval); break;
	case FAIL:cout << "\n分配失败！此次分配会导致系统进入不安全状态！！！\n"; Display_Security(retval); break;
	}
}
pair<vector<int>, Status> AlgoBanker(int Pid, vector<int>& Request)//银行家算法,传入进程Id和请求分配大小
{
	pair<vector<int>, Status> retval;//first---安全序列，second---分配状态

	if (!(Request <= Need[Pid]))//请求大于所需大小
	{
		retval.second = NEED_REQUEST;
	}

	else if (!(Request <= Available))//请求大于系统拥有资源
	{
...
pair<vector<int>, Status> AlgoSecurity()//安全性算法
{
	...
	vector<bool> Finish(Need.size(), false);//可分配标志，进程个false给finish
	int i, j;
	...
	while (1)
	{
		...
		for (i = 0; i < Need.size(); i++)//i小于进程的个数
		{
